...
from datasets_helper_files.build_dataset import make_face_transforms
...
def infer(images_path, model, postprocessors, device, output_path):
    ...
    for img_sample in images_path:
        ...
        op_logits = outputs["pred_logits"].squeeze()
        print("before softmax",op_logits.shape)
        
        
        probas = op_logits.softmax(dim=1)
        
        print("after softmax",probas.shape)
...
if __name__ == "__main__":
    ...
    if args.resume: 
        ...
        print("checkpoint model is = ",type(checkpoint['model']))
        for key in checkpoint['model'].items():
            print(key)
            break
